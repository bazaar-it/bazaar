  ...
  const createShare = api.share.createShare.useMutation({
    onSuccess: async (data) => {
      ...
      try {
        ...
        // If copy fails, show the URL in the button
  ...
  return (
    <header className="flex items-center justify-between px-6 py-3 w-full bg-background z-10" style={{ height: 68 }}>
      ...
      <div className="flex items-center gap-4 min-w-[180px] justify-end">
        ...
        {projectId && (
          shareUrl ? (
            // Show share URL as an input field when clipboard copy fails
            <div className="flex items-center gap-2">
              <Input
                value={shareUrl}
                readOnly
                className="w-[300px] h-8 text-sm font-mono rounded-[15px] shadow-sm pr-2"
                onClick={(e) => {
