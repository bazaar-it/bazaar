...
export default function ChatPanelG({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 🚀 SIMPLE APPROACH: Add loading message and trigger generation
    console.log('[ChatPanelG] 🚀 Adding loading message and starting generation...');
    
    // Add temporary loading message
    const tempMessageId = `temp-${Date.now()}`;
    addAssistantMessage(projectId, tempMessageId, 'Generating code...');
    activeAssistantMessageIdRef.current = tempMessageId;
    
    // Start generation asynchronously
    startGeneration(trimmedMessage, imageUrls, tempMessageId);
  };
  ...
  // Store active message ID for updates
  ...
  // Handle scene operations (extracted for clarity)
  const handleSceneOperation = useCallback((actualScene: any, operation: string) => {
    const currentScenes = getCurrentProps()?.scenes || [];
  ...
  }, [projectId, getCurrentProps, deleteScene, updateScene, onSceneGenerated, replace]);

  // Simple generation function to replace SSE
  const startGeneration = useCallback(async (userMessage: string, imageUrls: string[], tempMessageId: string) => {
    try {
      ...
      updateMessage(projectId, tempMessageId, {
        content: aiResponse,
        status: 'success'
      });
      ...
      if (actualScene) {
        handleSceneOperation(actualScene, operation);
      }
  ...
  }, [projectId, generateSceneMutation, updateMessage, handleSceneOperation]);

  // Reset generation complete state when project changes
  useEffect(() => {
    setGenerationComplete(false);
  }, [projectId]);
