...
export function initializeAgents(taskManager: TaskManager): BaseAgent[] {
  ...
  a2aLogger.info(null, "Initializing A2A Agents - first time initialization", { module: "agent_init"});
  ...
  try {
    ...
    try {
      ...
      a2aLogger.info(null, `Created ScenePlannerAgent successfully`, { module: "agent_creation"});
    } catch (error) {
      const err = error as any;
      a2aLogger.error(
        null,
