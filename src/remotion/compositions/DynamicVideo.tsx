...
} from "../../lib/types/video/input-props";

import { sceneRegistry, CustomScene } from "../components/scenes";
import type { Scene } from '~/lib/types/video/input-props';
// Only CustomScene needed - all scenes are generated custom code
...
export const DynamicVideo: React.FC<CompositionProps> = ({ 
  ...
  const getSceneComponent = (scene: Scene) => {
    ...
    // SIMPLIFIED: Only custom scenes supported (all generated by our pipeline)
    if (scene.type === 'custom') {
    ...
    // Fallback for any unexpected scene types
    console.warn(`[DynamicVideo] Unexpected scene type: ${scene.type}. All scenes should be 'custom'.`);
    return (
      <AbsoluteFill
        key={sceneKey}
        style={{
      ...
      >
        <h1>All scenes should be custom. Type: {scene.type}</h1>
      </AbsoluteFill>
    );
  };
