...
export class NotificationService {
  private readonly debounceWindow = config.patterns.debounceWindow;
  private readonly issueThreshold = config.patterns.issueThreshold;
   ...
   */
  async processIssue(issue: Issue, isNew: boolean): Promise<boolean> {
   ...
   */
  private async sendNotification(issue: Issue): Promise<boolean> {
   ...
   */
  shouldNotify(issue: Issue): boolean {
    // Check threshold
    if (issue.count < this.issueThreshold) {
      return false;
    }
