...
app.post('/qna', async (req, res) => {
    ...
    try {
        ...
        const { logs } = await redisService.getLogs(runId, undefined, // All sources
        undefined, // No filter
        ...
        // Analyze with OpenAI
        const response = await openaiService.analyzeLogs({ ...request, runId }, logs);
        const processingTime = Date.now() - startTime;
        // Return analysis
...
app.post('/control/clear', async (req, res) => {
    try {
        ...
        // Start new run
        const newRun = await redisService.startNewRun(request.newRunId || generateRunId(), request.callback);
