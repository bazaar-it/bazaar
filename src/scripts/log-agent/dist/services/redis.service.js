...
export class RedisService {
    ...
    createIsolatedClient() {
        ...
        isolatedClient.connect().catch((err) => {
     ...
     */
    async getLogs(runId, source, filter, limit = 100, offset = 0) {
        ...
        // Process results
        const counts = results.slice(0, keys.length);
        const logArrays = results.slice(keys.length);
    ...
    async storeIssue(issue) {
        ...
        const issues = existingIssuesStr ? JSON.parse(existingIssuesStr) : [];
     ...
     */
    async getIssues(runId, source, level, limit = 50, offset = 0) {
        // If runId is 'latest', get the most recent runId
        if (runId === 'latest') {
            const latestRunId = await this.getLatestRun();
            if (!latestRunId) {
        ...
        let issues = JSON.parse(issuesStr);
        // Apply filters
        if (source) {
     ...
     */
    async markIssueNotified(runId, fingerprint) {
        const key = redisKeys.issues(runId);
        // Get existing issues
        const existingIssuesStr = await this.client.get(key);
        if (!existingIssuesStr)
            return false;
        const issues = JSON.parse(existingIssuesStr);
     ...
     */
    async storeCallback(runId, callbackUrl) {
     ...
     */
    async startNewRun(newRunId, callbackUrl) {
