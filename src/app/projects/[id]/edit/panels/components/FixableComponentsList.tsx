//src/app/projects/[id]/edit/panels/components/FixableComponentsList.tsx
...
interface FixableComponentsListProps {
  projectId: string;
  components: ComponentJob[];
  isLoading: boolean;
  onFixComponent: (componentId: string) => void;
  isFixing: boolean;
  fixingComponentId?: string;
}

export function FixableComponentsList({ 
  components, 
  isLoading, 
  onFixComponent,
  isFixing,
  fixingComponentId 
}: FixableComponentsListProps) {
  ...
  if (!components || components.length === 0) {
    return null; // Hide component when no fixable components
  }
      ...
      <div className="space-y-3">
        {components.map((component) => (
          <FixableComponentCard 
            key={component.id}
            component={component}
            onFix={() => onFixComponent(component.id)}
            isFixing={isFixing && fixingComponentId === component.id}
          />
        ))}
      </div>
    </div>
