...
1. ALWAYS use 'frame' not 'currentFrame': const frame = useCurrentFrame()
2. NEVER duplicate fps in spring: spring({ frame, fps: 30, ... }) NOT spring({ frame, fps: 30, fps: 30, ... })
3. ONE Remotion destructure per scene at the top
4. UNIQUE variable names with suffixes to avoid collisions (e.g., padding_a8f3d2e1, not just padding)
5. NEVER start code with random characters like 'x' or streaming artifacts
...
- Use spring animations with fps parameter EXACTLY ONCE: spring({ frame, fps: 30, from: 0, to: 1 })
