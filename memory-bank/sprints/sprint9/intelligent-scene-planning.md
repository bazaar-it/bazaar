...
1. `src/app/projects/[id]/edit/panels/TimelinePanel.tsx` - Add UI feedback for scene generation
   - Progress indicators for scene generation
   - Visual feedback for scene status changes
   - Error display for failed scenes
   - Drag-to-chat functionality for editing specific scenes
   - Scene regeneration button

2. `src/server/api/routers/video.ts` - Update component insertion
   - Ensure dynamic durations are respected
   - Add support for repositioning subsequent scenes

## 8. LLM Communication Pattern
...
### 13.1 Model Standardization (2024-05-28)
