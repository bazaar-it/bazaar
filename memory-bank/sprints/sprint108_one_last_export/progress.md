...
## 2025-10-04 – Lambda Remotion bindings restored
- `preprocessSceneForLambda` now tracks destructured `window.Remotion` identifiers, and the Lambda runtime passes `Easing` while `MainCompositionSimple` exposes a shimmed `window.Remotion` with the same primitives. This covers scenes generated with `Easing` without reintroducing compilation churn.【src/server/services/render/render.service.ts:224】【src/remotion/MainCompositionSimple.tsx:5】【src/remotion/MainCompositionSimple.tsx:158】
- Targeted icon isolation Jest suite still fails on the longstanding `__InlineIcon` expectations; no regressions introduced (same failure existed pre-change), but we should modernize those assertions once the icon runtime stabilizes.
