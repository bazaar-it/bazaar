...
async function checkDatabase() {
  ...
  try {
    ...
    console.log(`📁 Projects in database: ${projectCount.rows[0]?.count || 0}`);
    ...
    console.log(`🔑 Accounts in database: ${accountCount.rows[0]?.count || 0}`);
    
    const sessionCount = await db.execute('SELECT COUNT(*) as count FROM "bazaar-vid_session"');
    console.log(`🎫 Sessions in database: ${sessionCount.rows[0]?.count || 0}`);
